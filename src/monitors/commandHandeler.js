...
module.exports = class extends Monitor {
	...
	parseCommand(message) {
		const result = this.customPrefix(message) || this.userPrefix(message) || this.mentionPrefix(message) || this.naturalPrefix(message) || this.prefixLess(message);
		return result ? {
	...
	userPrefix({ content, author: { settings: { prefixes } } }) {
		if (!prefixes.length) return null;
		for (const prf of Array.isArray(prefixes) ? prefixes : [prefixes]) {
			const testingPrefix = this.prefixes.get(prf) || this.generateNewPrefix(prf);
			if (testingPrefix.regex.test(content)) return testingPrefix;
		}
		return null;
	...
	generateNewPrefix(prefix) {
		...
		const prefixObject = { length: prefix.length, regex: new RegExp(`^${regExpEsc(prefix)}(?:\s)?`, this.prefixFlags) };
