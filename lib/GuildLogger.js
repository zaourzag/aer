...
class GuildLogger {
	...
	async send({ type, action, color, user, users, member, moderator, reason, duration, message, oldMessage, newMessage }) {
		...
		if (moderator)
			embed.setFooter(this.guild.language.get('LOG_ARGS_MODERATOR', moderator.tag || moderator.user.tag, moderator.id));
